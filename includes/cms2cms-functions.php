...
class CmsPluginFunctionsDnn
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback"  : "//wordpress.org/support/plugin/cms2cms-dnn-software-to-wp-migrator/reviews",
    ...
    {
        $cms2cms_dnn_access_login = get_option('cms2cms-dnn-login');
        $cms2cms_dnn_access_key   = get_option('cms2cms-dnn-key');

        return array(
            'email'     => $cms2cms_dnn_access_login,
            'accessKey' => $cms2cms_dnn_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_dnn_access_key != false);
    }
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-dnn-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-dnn-key', $key);

            delete_option('cms2cms-dnn-login');
            add_option('cms2cms-dnn-login', $login);

            delete_option('cms2cms-dnn-depth');
            add_option('cms2cms-dnn-depth', $bridge_depth);
    ...
    {
        return $this->__('Dnn', 'cms2cms-dnn-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-dnn-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-dnn-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-dnn-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if (isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_dnn_logout')
                && $_POST['cms2cms_dnn_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
